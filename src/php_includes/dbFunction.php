...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                foreach ($sections as $key => $value) {
                    ...
                    if (($value->status) == 1) {
                        ...
                        $log->info("update section sql 1= $sql");
    ...
    if ($data->procedureName == "deleteArticle") {

        $slug = $data->slug;

        $sql = "DELETE I FROM infocard I
        inner join articles A on I.article_id = A.id
        where A.Slug = '$slug';";

        $sql .= "DELETE S FROM sections S
        inner join articles A on S.article_id = A.id
        where A.Slug = '$slug';";

        $sql .= "DELETE FROM articles WHERE slug = '$slug'";

        $log->info("deleteArticle sql = $sql");

        if ($link->multi_query($sql)) { {
                // Store first result set
                if ($result = $link->store_result()) {
                    while ($row = $result->fetch_row()) {
                        $log->info("deleteArticle sql = $row[0]");
                    }
                    $result->free_result();
                }
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($procedureName == "getAllArticles") {
        ...
        summary, banner, is_featured, slug, A.is_active, A.created_at , S.status
        FROM articles A 
        inner join categories C on A.category_id = C.id
        inner join users U on A.user_id = U.id 
        join status S on A.status = S.id
        WHERE type = 2
        ORDER BY A.id DESC LIMIT $start_from,$limit";
    ...
    if ($procedureName == "getArticleSectionsBySlug") {
        ...
        $sql = "SELECT S.id, S.title, S.content as description, S.status, S.sortorder
        FROM articles A 
        inner join sections S on S.article_id = A.id
        where A.Slug = '$Slug' order by S.sortorder";
